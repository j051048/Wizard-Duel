...
const WagerSelector: React.FC<WagerSelectorProps> = ({
  ...
  return (
    <div id="lobby-wager-selector" className="flex flex-col items-center gap-2 w-full">
       ...
       <div className={`flex ${isMobile ? 'gap-3' : 'gap-4 md:gap-8'} justify-center w-full`}>
          {BET_OPTIONS.map((amt) => {
             ...
             return (
                <button
                   ...
                   disabled={isDisabled}
                   className={`
                      relative ${isMobile ? 'w-12 h-12' : 'w-16 h-16 md:w-20 md:h-20'} rounded-full flex items-center justify-center transition-all duration-300 group
                      ${isSelected 
                ...
                >
                   {/* Chip Visual */}
                   <div className={`absolute inset-0 rounded-full border-4 ${isSelected ? 'border-amber-400 bg-purple-900 shadow-[inset_0_0_10px_rgba(0,0,0,0.5)]' : 'border-slate-600 bg-slate-900 shadow-xl'}`}></div>
                   <div className="absolute inset-1 rounded-full border border-dashed border-white/20"></div>
                   
                   <span className={`relative z-10 font-black ${isMobile ? 'text-sm' : 'text-lg md:text-xl'} font-mono ${isSelected ? 'text-white' : 'text-gray-400'}`}>
                      {amt}
                   </span>
                   {isSelected && <div className="absolute -bottom-1.5 md:-bottom-2 left-1/2 -translate-x-1/2 w-1 h-1 bg-amber-400 rounded-full shadow-[0_0_10px_orange]"></div>}
                </button>
             );
          })}
       </div>
